...
class ModelVisualInfo:
    ...
    def _render(self, epoch_i, loss):
        ...
        # safe defaults if called from note()
        if epoch_i is None:
            epoch_i = 0
        if loss is None:
            loss = 0.0
    ...
    def progress_line(self, epoch_i: int, loss: float):
        self._render(epoch_i=epoch_i, loss=loss)
...
class EvalVisualInfo:
    ...
    def _render(self, *, grid_frac, me_frac):
        ...
        if grid_frac is not None:
            self._last_grid_frac = grid_frac
        grid_frac = self._last_grid_frac
