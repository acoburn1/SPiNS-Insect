...
def train_variety_and_save_data(data_preparer, num_epochs, num_features, hidden_layer_size, modular_reference_matrix, lattice_reference_matrix, filename):
    ...
    np.savez(filename, m_avgs=all_m_avgs, l_avgs=all_l_avgs, mpms=all_mpms, lpms=all_lpms, ngpms=all_gpms, losses=all_losses, batch_sizes=batch_sizes, learning_rates=learning_rates)

def load_and_display_data(num_epochs, modular_reference_matrix, lattice_reference_matrix, filename):
