...
def print_ratio_results(results: Dict[str, Dict[str, List[List[float]]]], result_type: str = "Output"):
    """
    Print the ratio trial results in a readable format.
    
    Args:
        results: Dictionary from test_ratios function with structure:
                {label: {"mod": [[correlations]], "lat": [[correlations]]}}
    ...
    for label, data in results.items():
        ...
        if num_trials > 1:
            print(f"\nFirst 3 trials (correlation with each exemplar):")
