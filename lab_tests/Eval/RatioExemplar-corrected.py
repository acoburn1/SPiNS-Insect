...
class RatioTestEvaluator:
    ...
    def run(self, cfg, zarr_path: str, vis=None) -> np.ndarray:
        ...
        for r in ratio_labels:
            for s in set_labels:
                ratio_set_ids[(r, s)] = _ids_for_conditions(
                    cfg.probe_index,
                    source="ratio",
                    ratio=r,
                    sets=s,
            ...
            for e in range(E):
                ...
                for r in ratio_labels:
                    ...
                    for s_idx, s in enumerate(set_labels):
                        ...
                        trials = reps[m, e, local, :]  # (T, H)
                        pref = _trial_prefers_mod(trials, mod_ex, lat_ex)  # (T,) bool

                        if pref.size == 0:
                            continue

                        out[m, e, r_idx, s_idx] = np.mean(pref.astype(np.float64, copy=False))
